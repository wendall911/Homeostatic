...
public class BodyTemperature {
    ...
    public BodyTemperature(ServerPlayer sp, EnvironmentData environmentData, ITemperature tempData) {
        this(sp, environmentData, tempData, false, false);
    }

    public BodyTemperature(ServerPlayer sp, EnvironmentData environmentData, ITemperature tempData, boolean updateCore, boolean updateSkin) {
    ...
    public void updateCoreTemperature(ServerPlayer sp) {
        ...
        if (this.skinTemperature < this.coreTemperature) {
            boolean hasFrostResistance = sp.hasEffect(HomeostaticEffects.FROST_RESISTANCE);
    ...
    private void setSkinTemperature(ServerPlayer sp, float skinTemperature, boolean updateSkin) {
            ...
            switch (skinTemperatureDirection) {
                ...
                case COOLING_NORMALLY -> {
                    ...
                    if (!canSweat) {
                        ...
                        //Homeostatic.LOGGER.debug("shivering ... adding exhaustion: {}, {}", exhaustion, foodLevel);
                    }
                }
                case WARMING -> {
                    if (canSweat) {
                ...
                case WARMING_NORMALLY -> {
                    if (canSweat) {
                        // Sweating
                        //Homeostatic.LOGGER.debug("WARMING_NORMALLY sweating: {}", Math.min(tempChange * 100.0F, 0.1F));
        ...
        if (localTemperature < Environment.PARITY_HIGH && !canSweat && Homeostatic.RANDOM.nextFloat() < 0.17F) {
            //Homeostatic.LOGGER.debug("Random cool water loss: {}", 0.05F);
        ...
        //Homeostatic.LOGGER.debug("update skin: {} {}", tempChange, skinTemperatureDirection);
    }
    ...
    public float getAirTemperatureSkinChange(ServerPlayer sp, double insulationModifier) {
        ...
        //Homeostatic.LOGGER.debug("{} {} {} {} {} {}", skinTemperatureDirection, localTempF, insulationModifier, change, temp, radiationModifier);
