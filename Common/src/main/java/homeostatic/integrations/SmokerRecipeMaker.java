...
public class SmokerRecipeMaker {
    ...
    public static List<RecipeHolder<SmokingRecipe>> createFlaskRecipes(String plugin) {
        String group = plugin + GROUP;
        List<RecipeHolder<SmokingRecipe>> recipes = new ArrayList<>();
        ...
        recipes.add(new RecipeHolder<>(
            prefix(group + ".flask"),
            new SmokingRecipe(group, CookingBookCategory.MISC, Ingredient.of(filledWaterLeatherFlask), filledPurifiedWaterLeatherFlask, 0.15F, 100)
        ));

        return recipes;
    }

    public static List<RecipeHolder<SmokingRecipe>> createWaterBottleRecipes(String plugin) {
        String group = plugin + GROUP;
        List<RecipeHolder<SmokingRecipe>> recipes = new ArrayList<>();
        ItemStack result = new ItemStack(HomeostaticItems.PURIFIED_WATER_BOTTLE);

        recipes.add(new RecipeHolder<>(
            prefix(group + ".water_bottle"),
            new SmokingRecipe(group, CookingBookCategory.MISC, Ingredient.of(RecipeProviderBase.waterBottle), result, 0.05F, 50)
        ));
