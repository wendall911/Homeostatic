...
public class SmeltingRecipeMaker {
    ...
    public static List<SmeltingRecipe> createFlaskRecipes(String plugin) {
        ...
        recipes.add(new SmeltingRecipe(loc(group + ".flask"), group, CookingBookCategory.MISC, Ingredient.of(filledWaterLeatherFlask), filledPurifiedWaterLeatherFlask, 0.15F, 150));

        return recipes;
    }

    public static List<SmeltingRecipe> createWaterBottleRecipes(String plugin) {
        String group = plugin + GROUP;
        List<SmeltingRecipe> recipes = new ArrayList<>();
        ItemStack result = new ItemStack(HomeostaticItems.PURIFIED_WATER_BOTTLE);

        recipes.add(new SmeltingRecipe(loc(group + ".water_bottle"), group, CookingBookCategory.MISC, Ingredient.of(RecipeProviderBase.waterBottle), result, 0.05F, 75));
