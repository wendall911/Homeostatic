...
public class BiomeCategoryProvider implements DataProvider {
    ...
    public BiomeCategoryProvider(DataGenerator dataGenerator, String modid) {
    ...
    protected void registerBiomeCategories() {
        ...
        add(Biomes.STONY_SHORE, BiomeCategory.Type.EXTREME_HILLS);
        ...
        add(Biomes.DEEP_FROZEN_OCEAN, BiomeCategory.Type.DEEP_COLD_OCEAN);
        ...
        add(ModIntegration.bopLoc("wetland"), BiomeCategory.Type.PLAINS);
        ...
        add(ModIntegration.bopLoc("muskeg"), BiomeCategory.Type.PLAINS);
        ...
        add(ModIntegration.bopLoc("wasteland"), BiomeCategory.Type.SAVANNA);
        ...
        add(ModIntegration.bopLoc("glowing_grotto"), BiomeCategory.Type.RIVER);
        add(ModIntegration.bopLoc("spider_nest"), BiomeCategory.Type.RIVER);
        ...
        add(ModIntegration.bygLoc("dead_sea"), BiomeCategory.Type.VOLCANIC);
    ...
    public void run(CachedOutput pOutput) throws IOException {
        ...
        for (Map.Entry<ResourceLocation, BiomeCategory> entry : BIOME_CATEGORY_MAP.entrySet()) {
            Path biomeCategoryPath = getPath(output, entry.getKey());

            try {
                DataProvider.saveStable(pOutput, BiomeCategoryManager.parseBiomeCategory(entry.getValue()), biomeCategoryPath);
            }
            catch (IOException e) {
                Homeostatic.LOGGER.error("Couldn't save homeostatic biome_category %s %s", biomeCategoryPath, e);
            }
        }
    }

    private static Path getPath(Path output, ResourceLocation loc) {
        return output.resolve("data/" + loc.getNamespace() + "/environment/biome_category/" + loc.getPath() + ".json");
    }
